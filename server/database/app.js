...
// Load JSON data
const reviews_data = JSON.parse(fs.readFileSync("./data/reviews_clean.json", "utf8"));
const dealerships_data = JSON.parse(fs.readFileSync("./data/dealers_clean.json", "utf8"));
...
mongoose.connect("mongodb://localhost:27017/dealerships")
  .then(() => {
    ...
    // Seed Reviews
    Reviews.deleteMany({})
      .then(() => Reviews.insertMany(reviews_data))
      .then(() => Reviews.countDocuments())
      .then(count => console.log("Reviews seeded:", count))
      .catch(err => console.error("Error loading reviews:", err));

    // Seed Dealerships
    Dealerships.deleteMany({})
      .then(() => Dealerships.insertMany(dealerships_data))
      .then(() => Dealerships.countDocuments())
      .then(count => console.log("Dealerships seeded:", count))
